...
1. `sources.ipynb`: Constructing the data used in the analysis. The binning scheme is described and data is constructed from 2FIG candidates and 3FGL sources.
2. `efficiency.ipynb`: Constructing of the pulsar detection efficiency for binned data in several ways.
3. `model.ipynb`: Deriving the modeled number of disk and bulge sources in each bin.
4. `likelihood.ipynb`: Calculating the likelihood for a given set of disk and bulge model parameters.
5. `scan.ipynb`: Performing the Bayesian scan and analyzing it to derive the best-fit disk and bulge model parameters.
