...
cpdef double Nbulge_full_ang_ijk(int i, int j, int k, double Nbulge, double omega_ijk, double alpha, double beta,double rcut , double Lmin, double Lmax ,int Ns ,int Nang, double b_mask ) nogil:
...
@cython.cdivision(True)
@cython.initializedcheck(False)
cpdef double Ndisk_full_ang_ijk(int i, int j, int k, double Ndisk, double omega_ijk, double n, double sigma,double z0, double beta, double Lmin, double Lmax ,int Ns ,int Nang ,double smax, double b_mask ) nogil:
    """ Return the number of bulge PSRs in bin (i,j,k) for given parameters
    ---arguments---
    Ns : number of points in s integration
    ----units---
